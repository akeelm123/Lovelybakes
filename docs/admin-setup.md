...
When email is configured, a committed payment, expiry, refund, or administrator fulfilment event immediately attempts all due messages for that order. Provider failure never reverses the order event: the failed message and its next-attempt time remain visible in `/admin/notifications` for owner retry. Replayed Stripe webhooks also retry any due message with the same provider idempotency key. Synthetic orders created by the admin test button remain queued and are not sent automatically.
